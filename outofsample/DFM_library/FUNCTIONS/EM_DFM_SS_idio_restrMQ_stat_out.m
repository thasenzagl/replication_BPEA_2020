function Res = EM_DFM_SS_idio_restrMQ_stat_out(X,Par,Res_old)

% EM algorithm for estimation of dynamic factor model with AR(1) idiosyncratic component
% and mixed frequency (monthly and quarterly) data
% written for 
...
thresh = 1e-4; % threshold for checking the convergence
r = Par.r; % number of factors
p = Par.p; % number of lags in factor VAR
max_iter = Par.max_iter; % maximal number of EM iterations
R_mat = Par.Rconstr; % matrix of constraints on the loading of quarterly data
q = Par.q;
% number of quarterly series, they are ordered as last
nQ = Par.nQ;
...
%y = C*Z + e;
%Z = A*Z(-1) + v
%where y is NxT, Z is (pr x 5r + N)xT, etc
...
% loadings for the monthly variables
nM = n-nQ;
...
% loadings for quarterly variables
...
for i=n-nQ+1:n
    ...
    nom = zeros(1,rC);
       ...
       idx_jQ = i-nM;
       i_idio_jQ = (rpC+nM+3*(idx_jQ-1)+1:rpC+nM+3*idx_jQ);
    ...
    for t=1:T
        ...
        nom = nom + y(i,t)*Zsmooth(1:rC,t+1)';
            nom = nom -...
                nanYt*([1 1 1]*Zsmooth(i_idio_jQ,t+1)*Zsmooth(1:rC,t+1)'+...
                [1 1 1]*Vsmooth(i_idio_jQ,1:rC,t+1));
    end
...
% initial parameter estimates for the EM algorithm
...
% loadings for the quarterly variables
...
% idiosyncratic component for the monthly data
for i = 1:NM;
    res_i = resNaN(:,i);
...
% idiosyncratic component for the quarterly data
C = [C [zeros(NM,3*NQ);kron(eye(NQ),[1 1 1])]];
Rdiag = diag(R);
sig_e = Rdiag(NM+1:N)/19;
R = 1e-16*eye(N);
rho0 = 0.1;
BQ = kron(eye(NQ),[[rho0 zeros(1,2)];[eye(2),zeros(2,1)]]);
temp = zeros(3);
temp(1,1) = 1;
SQ = kron(diag((1-rho0^2)*sig_e),temp);
initViQ = reshape(inv(eye((3*NQ)^2)-kron(BQ,BQ))*SQ(:),3*NQ,3*NQ);
...
% combine all the parameters for the transition equation
A = blkdiag(A, BM, BQ);
Q = blkdiag(Q, SM, SQ);
