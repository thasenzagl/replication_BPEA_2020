...
% Recession Dates
global_and_fin.recessions.r1_start = find(global_and_fin.name_vintage >= datenum('01-01-2008'), 1, 'first');
global_and_fin.recessions.r1_end = find(global_and_fin.name_vintage <= datenum('04-01-2009'), 1, 'last');
...
title('Global, Financial and Non-financial Factors', 'Fontsize', 18)

legend({'Global Factor','Financial Factor','Non-financial Factor'}, 'Location', 'SouthWest', 'FontSize', 14)
set(gcf,'Position',[800 450 800 450])
...
legend({'Global Factor','Global and Financial Factors','Non-financial Factor'}, 'Location', 'SouthEast', 'FontSize', 14)
datetick('x', 'yyyy');
set(gcf,'Position',[800 450 800 450])
...
ylim([min(global_and_fin.meanSE(20:end))-0.1,max(global_and_fin.meanSE(20:end))+0.1])
set(h1, 'Xdir', 'reverse');
set(gcf,'Position',[800 450 800 450])
...
xlabel('Distance to Release (days)', 'FontSize', 14)
ylabel('Predictive Score', 'FontSize', 14)
title('Predictive Score', 'Fontsize', 18)
legend({'Global Factor','Global and Financial Factors','Non-financial Factor'}, 'Location', 'SouthEast', 'FontSize', 14)
...
load("output/matlab_output/output_" + mode, "PST_bc", "PST_nc", "PST_fc", ...
    ...
    "datesQ", "factor_fin", "factor_global", "y");

load("data/real_time_data", "target_releases", "Period");
...
out.target_releases = target_releases;
...
target_releases = struc.target_releases;
distance = 199:-1:0;
...
for t = 1:size(struc.name_vintage,1)
    ...
    for i=-3:3:3
        if i_ref+i <= size(struc.Period,1)
